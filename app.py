...
if input_source == "🔴 Live Webcam Feed":
    ...
    with col_live:
        ...
        if run_live:
            ...
            cap, backend = open_webcam()
            if cap is None or not cap.isOpened():
                st.error(
                    "❌ Error: Webcam not detected. "
                    "On Windows, ensure `opencv-python` is installed instead of `opencv-python-headless`."
                )
            else:
                ret, frame = cap.read()
                if not ret:
            ...
            st.info("Click the checkbox above to start the webcam feed.")
